...
export default function PostModal({
            ...
            <div className="absolute right-0 top-full mt-1 bg-[var(--card)] border border-[var(--border)] rounded-lg shadow-lg py-1 min-w-[140px] z-10">
              {COMMENT_FILTER_OPTIONS.map((option) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs hover:bg-[var(--muted)] transition-colors ${commentFilter === option.value ? 'bg-[var(--muted)] font-medium' : ''
                    }`}
                >
                  {option.label}
                </button>
    ...
    <div className="flex-shrink-0 bg-[var(--card)] border-t border-[var(--border)]">
      ...
      >
        <input
          ...
          className="flex-1 bg-transparent outline-none text-sm p-2 h-10"
        />
                <div className={`text-xs text-[var(--muted-foreground)] mt-1 text-right ${mainCommentForm.content.length > 10000 && "text-red-500"}`}>
                        {mainCommentForm.content.length}/10000
                    </div>
  ...
  return (
    ...
    >
      <div
        className={`flex flex-col w-full  ${hasMedia ? "md:flex-row h-[90vh]" : "h-[80vh]"
